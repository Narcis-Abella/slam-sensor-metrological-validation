...
**Duration:** 2–4 h (sufficient to capture thermal stabilization)  
...
2. **Thermal characterization:** The cooling curve (temperature vs. time) documents the thermal time constant of each sensor — a parameter relevant to Phase II modeling
...
3. At each pose: record flange pose from RobotStudio + sensor pose estimate relative to a static reference (AprilTag board fixed in the environment)
4. Solve AX = XB for the rigid transform X between flange and sensor center
5. Verify: reprojection error on held-out poses

**Tooling:** `easy_handeye` (ROS) or equivalent  
