...
4. [Kinematic Residual Extraction](#4-kinematic-residual-extraction)
5. [Residual Comparison Pipeline (Primary)](#5-residual-comparison-pipeline-primary)
6. [CW/CCW Asymmetry Analysis](#6-cwccw-asymmetry-analysis)
7. [Frequency-Sweep Trajectory (Optional)](#7-frequency-sweep-trajectory-optional)
8. [Analysis Toolchain Summary](#8-analysis-toolchain-summary)
...
- **Condition R:** real hardware sensor residuals ($\text{measured} - \text{true}_\text{YuMi}$, per §2.1 and §4.1)
- **Condition M:** simulated sensor residuals under the metrological simulation ($\text{sim output} - \text{sim true}$, per §5)
...
- CW/CCW asymmetry per session (§6)
- Secondary noise model rungs M1 and M3 at T2

---

## 4. Kinematic Residual Extraction
...
Noise model definitions M1–M4 and kinematic regime taxonomy S0–S3 are defined in README §4 and RESEARCH_PLAN §2 respectively.
...
### 4.2 Kinematic Segmentation

Residuals are analysed in segments corresponding to different kinematic regimes (S0-S3).

This segmentation makes it possible to quantify how noise statistics change with motion intensity.

### 4.3 Dynamic Noise Model (M4): Goal and Thermal Component
...
### 4.4 Candidate Formulas for the Kinematic Term (to validate empirically)
...
#### 4.4.1 Sensor-specific coefficient pruning
...
### 4.5 Sensor-Specific Instantiation
...
### 4.6 M4 Generalization Check (held-out trajectory)
...
## 5. Residual Comparison Pipeline (Primary)
...
Pipeline Step 1: Real hardware data collection
  ...
  → CAD-derived T_sensor→flange: sensor true kinematics expressed in sensor frame (§4.1)
  → Residual computation: R_real(t) = measured(t) - true_YuMi(t), per §2.1
  → Kinematic segmentation into S0-S3 regimes (§4.2)
  → M4 coefficient fitting on T1 + T2 residuals (§4.3-4.4)
...
Pipeline Step 3: TOST equivalence testing
  ...
  → T3 residuals evaluated as held-out generalization check for M4 (§4.6)
```
...
## 7. Frequency-Sweep Trajectory (Optional)
...
| IMU residual computation | Custom Python (spline differentiation + CAD T_sensor→flange) | Cubic B-spline on SE(3) flange poses; residuals per §4.1 |
