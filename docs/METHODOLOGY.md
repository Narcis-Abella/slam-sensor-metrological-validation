...
| **Cable routing effects** | Systematic (variable per trajectory) | Estimated < 0.5 mm at T1; higher at T2/T3 | USB 3.0 and Ethernet cables routed along YuMi arm introduce variable external forces on the flange during dynamic trajectories. Mitigated by cable chain routing; residual effect included in budget. A dedicated pre-calibration test without payload cables will quantify the upper bound of this specific mechanical error before main data collection. |
...
**Equivalence margin $\delta$:** The margin is defined in units of Absolute Trajectory Error (ATE). To ensure clinical/practical relevance, the margin $\delta$ must be defined *a priori* — based on specific system requirements (e.g., maximum navigation tolerance for a mobile platform) and agreed upon with the supervisor **before** inspecting any empirical data. It is explicitly NOT defined as a percentage of the measured error, as this would invalidate the equivalence test by making the hypothesis data-dependent. Sensitivity to alternative margin choices (e.g., $\delta/2$ and $2\delta$) will be reported in the Discussion.

**Power analysis and sample size:** Each 2 h block yields **$n \approx 40$–60** independent repetitions (60 s static → 1–2 min trajectory → 60 s static per repetition). With three blocks per trajectory type, **$n \approx 120$–180** per trajectory type. Once the equivalence margin $\delta$ and the expected standard deviation ($\sigma$) are formally defined, an explicit statistical power analysis ($1-\beta$) will be calculated to verify that the sample size is sufficient to achieve at least 80% power at $\alpha=0.05$.
